...
public:
    ...
    EncodedResults generate(
        ...
        std::vector<std::vector<float>> plain_log_probs;
        std::vector<float> plain_scores;
        for (EncodedGenerationResult& res : generated) {
            ...
            std::move(res.m_scores.begin(), res.m_scores.end(), std::back_inserter(plain_scores));
            std::move(res.m_log_probs.begin(), res.m_log_probs.end(), std::back_inserter(plain_log_probs));
        }
        ...
        return {std::move(plain_tokens), std::move(plain_scores), std::move(plain_log_probs), std::move(perf_metrics), generated[0].extended_perf_metrics};
    }
