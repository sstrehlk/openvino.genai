...
void fill_prompt_log_probs(std::vector<SequenceGroup::Ptr>& sequence_groups, ov::Tensor& logits) {
    ...
    for (size_t sequence_group_id = 0, currently_processed_tokens = 0; sequence_group_id < sequence_groups.size(); ++sequence_group_id) {
        ...
        std::cerr << "[DEBUG fill_prompt_log_probs] prompt_len=" << prompt_len 
        ...
        for (int offset = 0; offset < prompt_len - 1; offset++) {
            ...
            int64_t token_id = sequence_group->get_prompt_ids()[offset + 1];  // Next token
            ...
            if (offset < 3) {  // Print first 3 for debug
                std::cerr << "[DEBUG] offset=" << offset << ", token_id=" << token_id 
                          << ", log_prob=" << (token_logit - max_value - log_sum) << std::endl;
                std::cerr.flush();
            }
        }

        std::cerr << "[DEBUG] After loop, appended " << (prompt_len - 1) << " log_probs" << std::endl;
        std::cerr.flush();
